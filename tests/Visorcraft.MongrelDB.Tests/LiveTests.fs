...
type LiveTests () =
    ...
    member _.``put then count round-trips`` () =
        ...
        try
            freshTable c name [| intCol 1 "id" true; floatCol 2 "amount" |]
            let r1 = Dictionary<int, obj>(); r1.[1] <- box 1; r1.[2] <- box 99.5
            c.Put(name, r1) |> ignore
            let r2 = Dictionary<int, obj>(); r2.[1] <- box 2; r2.[2] <- box 150.0
    ...
    member _.``upsert inserts then updates`` () =
        ...
        try
            freshTable c name [| intCol 1 "id" true; floatCol 2 "amount" |]
            let upd = Dictionary<int, obj>(); upd.[2] <- box 99.5
            let r1 = Dictionary<int, obj>(); r1.[1] <- box 1; r1.[2] <- box 99.5
            c.Upsert(name, r1, upd) |> ignore
            Assert.Equal(1L, c.Count(name))
            let upd2 = Dictionary<int, obj>(); upd2.[2] <- box 120.0
            let r2 = Dictionary<int, obj>(); r2.[1] <- box 1; r2.[2] <- box 120.0
    ...
    member _.``delete_by_pk removes the row`` () =
        ...
        try
            ...
            let r = Dictionary<int, obj>(); r.[1] <- box 5
            c.Put(name, r) |> ignore
            Assert.Equal(1L, c.Count(name))
    ...
    member _.``delete by row id removes the row`` () =
        ...
        try
            freshTable c name [| intCol 1 "id" true |]
            let r = Dictionary<int, obj>(); r.[1] <- box 7
            c.Put(name, r) |> ignore
    ...
    member _.``query by primary key returns one row`` () =
        ...
        try
            freshTable c name [| intCol 1 "id" true |]
            let r1 = Dictionary<int, obj>(); r1.[1] <- box 42
            c.Put(name, r1) |> ignore
            let r2 = Dictionary<int, obj>(); r2.[1] <- box 43
            c.Put(name, r2) |> ignore
            let cond = Dictionary<string, obj>(); cond.["value"] <- box 42
    ...
    member _.``query range with friendly aliases filters correctly`` () =
        ...
        try
            ...
            let mk a b =
                let r = Dictionary<int, obj>(); r.[1] <- box a; r.[2] <- box b; r
    ...
    member _.``transaction put commit`` () =
        ...
        try
            ...
            for i in 1 .. 3 do
                let r = Dictionary<int, obj>(); r.[1] <- box i
    ...
    member _.``transaction commit with idempotency key does not double-apply`` () =
        ...
        try
            ...
            let txn = c.BeginTransaction()
            let r = Dictionary<int, obj>(); r.[1] <- box 100
            txn.Put(name, r) |> ignore
            ...
            let r2 = Dictionary<int, obj>(); r2.[1] <- box 100
    ...
    member _.``transaction rollback discards ops`` () =
        ...
        try
            freshTable c name [| intCol 1 "id" true |]
            let txn = c.BeginTransaction()
            let r1 = Dictionary<int, obj>(); r1.[1] <- box 1
            let r2 = Dictionary<int, obj>(); r2.[1] <- box 2
    ...
    member _.``transaction double commit raises`` () =
        ...
        try
            freshTable c name [| intCol 1 "id" true |]
            let txn = c.BeginTransaction()
            let r = Dictionary<int, obj>(); r.[1] <- box 1
            txn.Put(name, r) |> ignore
            txn.Commit() |> ignore
    ...
    member _.``duplicate put with a UNIQUE constraint raises ConflictException`` () =
        ...
        try
            ...
            let err = Assert.Throws<ConflictException>(fun () ->
                let r2 = Dictionary<int, obj>(); r2.[1] <- box 1
                c.Put(name, r2) |> ignore)
