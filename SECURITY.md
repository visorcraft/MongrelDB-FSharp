...
This document describes the security properties of the MongrelDB F# client
and how to report vulnerabilities.

## Overview

The MongrelDB F# client is a pure F# library (no native dependencies) that
talks to `mongreldb-server` over HTTP using the .NET base-class-library
`HttpClient` and `System.Text.Json`. The client itself holds no encryption
...
- Server and network errors are mapped to the typed exception hierarchy
  ...
  exceptions.
- NaN and Infinity raise a clear `QueryException` instead of corrupting data;
  the `/sql` endpoint's Arrow IPC bodies are tolerated gracefully.

## Dependency security

The MongrelDB F# client has no runtime dependencies beyond the .NET 8 base
...
- the MongrelDB F# client version, .NET runtime version, and OS,
